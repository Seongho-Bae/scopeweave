...
function groupTimelineByWeek(days) {
  const groups = [];
  // ⚡ Bolt: Replace O(N^2) Array.find with O(1) Map lookup for grouping timeline
  const mapByMonday = new Map();
  days.forEach((day) => {
    const monday = getMonday(day.date);
    let existing = mapByMonday.get(monday);
    if (existing) {
      ...
      mapByMonday.set(monday, newGroup);
    }
  });
  return groups;
}
