...
function renderTaskRow(task, taskMetrics, ownerColorMap, index) {
  ...
  const addChildTitle = isLeaf ? '최대 3단계까지만 추가할 수 있습니다.' : '하위 작업 추가';
    ...
    <tr class="task-row depth-${task.depth} ${index % 2 === 1 ? 'striped-even' : ''}" data-task-id="${escapeHtml(task.id)}" draggable="true">
      <td>
        <div class="action-stack">
          ${toggleButton}
          <button type="button" class="icon-button" data-action="add-child" aria-label="하위 추가" title="${addChildTitle}" ${isLeaf ? 'disabled' : ''}>＋</button>
