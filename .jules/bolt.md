...
## 2026-06-26 - O(N) `.toLocaleString()` bottleneck in formatting loops
**Learning:** Calling `Number(value).toLocaleString('ko-KR')` inside an O(N) loop (like `renderAll` generating table cell content) creates a noticeable performance bottleneck because the browser has to dynamically resolve the locale, fetch data, and instantiate a new formatter object on every single function call.
**Action:** Always instantiate an `Intl.NumberFormat('ko-KR')` object once in the global or module scope and call its `.format()` method inside the loop. This caches the locale data and drastically reduces memory allocation and execution time.
