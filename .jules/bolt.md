...
## 2024-05-18 - Optimize dragover array searches
**Learning:** High-frequency event listeners like `dragover` that perform dual linear searches (`state.tasks.find`) to locate a dragged item and a drop target item cause massive main thread CPU usage, because they fire repeatedly per second with identical arguments. Caching based strictly on reference can be dangerous in an application with mutable data where tasks might be reordered or replaced.
**Action:** When handling linear array lookups inside rapid events, use a small Most Recently Used (MRU) index cache (storing the target id and array index) to turn O(N) into O(1). Always include a safety check (e.g., `state.tasks[cachedIndex]?.id === targetId`) before returning the cached item to guarantee safety against underlying array mutations.
