...
## 2026-06-28 - Optimize Memory Allocation in Rendering Loops
**Learning:** Instantiating `new Set()` inside high-frequency rendering loops (e.g. `getVisibleTasks()` and `renderAll()`) causes hidden O(N) memory allocation overhead on every render, leading to frequent garbage collection and potential UI stutter in JavaScript applications without a Virtual DOM diffing engine.
**Action:** Always instantiate sets, maps, or reusable arrays outside the main execution loop (as globals or module scope cache) and use `.clear()` or `.length = 0` to reuse the existing object reference, minimizing garbage collection pauses.
