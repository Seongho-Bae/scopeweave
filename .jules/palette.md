...
## 2026-06-29 - Keyboard Shortcut Hints in UI
**Learning:** Adding keyboard shortcut hints to primary action buttons (like Save and Cancel) improves discoverability for power users when the hints are exposed visually and through accessibility metadata.
**Action:** Consistently add `title` and `aria-keyshortcuts` attributes to primary actions (e.g., `Enter`) and cancel actions (e.g., `Escape`) across form and dialog components.
