...
## 2026-06-29 - Add skip-to-content link
**Learning:** Keyboards users need a way to bypass repetitive top-level navigation, particularly in complex UI layouts like a WBS planner where forms or tables contain a lot of inputs. A visually hidden skip link that becomes visible on focus provides immediate navigation to the main content.
**Action:** Add a `<a class="skip-link" href="#main-content">` right after the opening `<body>` tag and ensure the `<main>` element has `tabindex="-1"` so it can programmatically receive focus without interfering with normal tab order.
