...
## 2026-06-29 - Form Input Hints, Tooltips, and Skip Links
**Learning:** Providing explicit placeholder examples in text fields prevents user hesitation by showing the expected format. Explicitly surfacing keyboard shortcuts (like Enter for Save and Esc for Cancel) in tooltips aids discoverability for power users. Furthermore, a "skip-to-content" link at the beginning of the document drastically improves keyboard navigation by allowing users to bypass a dense top panel.
**Action:** When adding or updating form inputs, include helpful placeholders. Always document keyboard shortcuts via the `title` attribute for buttons if they are supported. Add skip links for main content areas on complex layouts.
