...
## 2026-06-26 - Placeholder text & Title for Context
**Learning:** Empty text inputs without placeholders fail to provide examples to users, and static document titles do not provide screen readers and multi-tab users context of what project they are currently viewing. Adding hints (Esc to close) on icon buttons helps keyboard users.
**Action:** When possible, inject helpful `placeholder` attributes into form fields to guide input formats (e.g. "예: 공통"). Dynamically update `document.title` to include relevant context (e.g., the current Project Name) when core application state changes.
