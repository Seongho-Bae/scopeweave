...
## 2026-06-28 - Gantt Bar Tooltips & Focus Context
**Learning:** In highly interactive tables (like WBS task lists), visual context is easily lost during keyboard navigation because `:hover` styles don't apply. Also, detached visualizations (like a Gantt chart without row headers) lack context for screen reader users and visual users alike unless explicit labels or tooltips are provided.
**Action:** Always use `:focus-within` on container rows/elements alongside `:hover` to maintain visual context for keyboard users. For detached visual elements (like Gantt bars), inject rich `title` or `aria-label` attributes to ensure the element's meaning is clear out-of-context.
