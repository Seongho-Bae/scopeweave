...
**Learning:** Native `disabled` attributes swallow all DOM events (including clicks) and prevent the element from receiving focus. This breaks keyboard accessibility because users tabbing through the page skip the element entirely. It also prevents us from attaching click event listeners to show helpful 'toast' tooltips explaining *why* a button is disabled. In this app, the original author attempted to show a toast when `add-child` or `connectJsonSync` were disabled, but those toasts were never visible because the clicks were swallowed by the browser.
**Action:** Use `aria-disabled="true"` instead of `disabled` for interactive buttons where we want to provide inline validation or a toast explaining the disabled state. Control the visual presentation via `[aria-disabled="true"]` in CSS and manually check `getAttribute('aria-disabled') === 'true'` in the click handler to `e.preventDefault()` and present the toast.
