...
## 2026-06-26 - Keyboard Shortcut Hints for Consistency
**Learning:** While some modal dialogs may have explicit keyboard shortcut hints (like "Esc" for closing), failing to provide the same hints in inline editors can cause an inconsistent UX. Adding these hints in `title` attributes improves discoverability for power users.
**Action:** Consistently add `title` attributes with keyboard shortcuts to primary actions (e.g., `(Enter)`) and cancel actions (e.g., `(Esc)`) across all form and dialog components.
