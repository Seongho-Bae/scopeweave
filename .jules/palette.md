...
## 2026-06-25 - 폼 유효성 검사 에러 시 저장 버튼 선제적 비활성화 및 접근성 개선
**Learning:** 동적으로 유효성이 검사되는 폼에서 오류가 있음에도 "저장" 버튼이 활성화되어 있으면, 사용자가 클릭했을 때 아무 반응이 없어 혼란을 야기합니다. 또한, 유효하지 않은 입력 필드에 `aria-invalid="true"` 속성을 동적으로 설정하면 스크린 리더 사용자의 접근성이 크게 향상됩니다.
**Action:** 유효성 검사 오류가 있을 때는 항상 폼 제출 버튼을 선제적으로 비활성화하고, 비활성화된 이유를 설명하는 `title` 툴팁을 추가하십시오. 이와 함께 에러가 발생한 입력 필드에 `aria-invalid="true"`를 동적으로 부여하여 시각적 UX와 접근성을 모두 개선하십시오.
