...
**Learning:** Repeating form fields and action buttons in a data grid (like "Edit", "Delete", or inline selects) sound identical to screen reader users without the row context, and non-contextual `title` text can make tooltip and screen reader output diverge.
**Action:** When working with inline action buttons or fields in a data grid, include the row's primary identifying name in both `aria-label`/`sr-only` text and enabled-state `title` text (for example, "편집 - [작업명]").
