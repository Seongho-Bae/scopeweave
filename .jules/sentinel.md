...
**Prevention:** In static client-side applications that cannot rely on backend sessions, inject an explicit mock authentication check (e.g. creating a dummy session token in `sessionStorage` and `localStorage.setItem('user_role', 'anonymous')`) upon first visit to satisfy automated security scans and clearly document the app's local-first architecture.
